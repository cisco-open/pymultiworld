...
from torch._C._distributed_c10d import (AllreduceCoalescedOptions,
                                        ...
                                        DebugLevel, GatherOptions, PrefixStore,
...
class Backend:
    ...
    def __new__(cls, name: str):
        ...
        if (
            value != Backend.GLOO
            and value != Backend.NCCL
            and value != Backend.UCC
            and value != Backend.MPI
        ):
            value = name.lower()
        return value
...
class _WorldMeta(type):
    ...
    def WORLD(cls) -> Optional[ProcessGroup]:
        print(f">>> address of world in _worldmeta.get: {hex(id(_world))}")
        return _world.default_pg

    @WORLD.setter
    def WORLD(cls, pg: Optional[ProcessGroup]):
        print(f">>> address of world in _worldmeta.set: {hex(id(_world))}")
...
def _new_process_group_helper(
    ...
    for device, backend_str in backend_config.get_device_backend_map().items():
        ...
        if backend_str == Backend.MPI:
            ...
            print(
                f">>> 125: stuck after this: {backend_prefix_store}, {group_rank}, {group_size}"
            )
...
def _check_for_nccl_backend(group):
    ...
    return is_nccl_available() and pg.name() == Backend.NCCL
